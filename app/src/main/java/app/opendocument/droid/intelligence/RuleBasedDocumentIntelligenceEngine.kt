...
/**
 * Small dependency-free fallback engine used while the local LLM/Collabora adapters are being
 * wired. It never claims to understand more than it can verify from text.
 */
class RuleBasedDocumentIntelligenceEngine : DocumentIntelligenceEngine {
        ...
        return when (request.action) {
            ...
            DocumentAction.SUMMARISE -> summarise(text)
                ...
                DocumentIntelligenceResult(
                    title =
                        request.action.name.replace('_', ' ').lowercase().replaceFirstChar {
                            it.uppercase()
                        },
                    ...
                    suggestedNextActions =
                        listOf(DocumentAction.SUMMARISE, DocumentAction.IDENTIFY_DOCUMENT),
                )
        }
    }
    ...
    private fun identify(text: String): DocumentIntelligenceResult {
        ...
        val type =
            when {
                listOf("balance sheet", "income statement", "profit before tax", "revenue").count {
                    it in lower
                } >= 2 -> "Financial statements"
                listOf("facility", "financing", "tenure", "security", "conditions precedent")
                    .count { it in lower } >= 2 -> "Financing / credit document"
                listOf("agreement", "party", "termination", "effective date").count {
                    it in lower
                } >= 2 -> "Contract / agreement"
                listOf("invoice", "amount due", "tax invoice").count { it in lower } >= 2 ->
                    "Invoice"
    ...
    private fun findDates(text: String): DocumentIntelligenceResult {
        val dateRegex =
            Regex(
                "\\b(?:[0-3]?\\d[-/][01]?\\d[-/](?:19|20)?\\d{2}|[0-3]?\\d\\s+(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\\s+(?:19|20)\\d{2})\\b",
        ...
        return DocumentIntelligenceResult(
            title = "Dates found",
            answer =
                if (matches.isEmpty()) "No explicit dates were found."
    ...
    private fun findActionItems(text: String): DocumentIntelligenceResult {
        ...
        return DocumentIntelligenceResult(
            ...
            answer =
                if (candidates.isEmpty()) "No explicit action items were detected."
    ...
    private fun findRisks(text: String): DocumentIntelligenceResult {
        ...
        return DocumentIntelligenceResult(
            ...
            answer =
                if (hits.isEmpty()) "No obvious keyword-based risk signals were found."
                else hits.joinToString("\n• ", prefix = "• "),
            confidence = Confidence.LOW,
            warnings = listOf("This is a screening aid, not a credit/legal conclusion."),
            suggestedNextActions =
                listOf(DocumentAction.ASK_DOCUMENT, DocumentAction.CHECK_INCONSISTENCIES),
        )
    }
}
